defmodule AuthToolkit.RateLimiterTest do
  ...
  describe "ETS rate limiter" do
    ...
    test "cleans up old entries" do
      ...
      future_time = old_now + 60_000 * 6 # 6 minutes into future
  ...
  describe "automatic cleanup" do
    ...
    test "keeps recent entries during cleanup" do
      ...
      with_mock System, [system_time: fn(:millisecond) ->
        ...
        case :erlang.get(:mock_time_called) do
          nil ->
            ...
            now
          _ ->
            now + 60_000 * 8  # 8 minutes in future (within 10 minute window)
        end
      end] do
        # Add initial entry
        ETS.check_rate("login", "test_user")
    ...
    test "partially cleans up old timestamps but keeps recent ones" do
      ...
      with_mocks [
        {System, [], [system_time: fn(:millisecond) -> now - 60_000 * 12 end]}  # 12 minutes ago
      ] do
        ETS.check_rate("login", "test_user")
      end

      with_mocks [
        {System, [], [system_time: fn(:millisecond) -> now - 60_000 * 4 end]}  # 4 minutes ago
      ] do
        ETS.check_rate("login", "test_user")
      end
